...
export function NotesPanel({
  ...
  useEffect(() => {
    const handleDocumentClick = (event: MouseEvent) => {
      ...
      if (
        headerGroupMenuRef.current &&
        !headerGroupMenuRef.current.contains(event.target as Node)
      ) {
        ...
        {sortedNotes.length === 0 ? (
          ...
          sortedNotes.map((note) => {
            ...
            const shouldShowExpandToggle =
              note.content.length > 220 || note.content.split("\n").length > 4;
            ...
            return (
              ...
              >
                ...
                {editingNoteContentId === note.id ? (
                  ...
                  <>
                    <p
                      className={`text-sm whitespace-pre-wrap break-words mb-2 ${isExpanded ? "" : "line-clamp-4"}`}
                    >
                      {note.content}
                    </p>
                ...
                {editingNoteTagId === note.id && (
                  <div className="space-y-2 mb-2">
                    <TagInput
                      ...
                      tagColors={tagColors}
                      onChange={(tags) =>
                        setTagDraftsByNoteId((prev) => ({ ...prev, [note.id]: tags }))
                      }
                ...
                {note.imageUrl && (
                  <div className="mt-2 rounded overflow-hidden border border-border">
                    <img src={note.imageUrl} alt="Note attachment" className="max-w-full h-auto" />
                  </div>
                )}
                ...
                requestAnimationFrame(() => {
                  if (noteTextareaRef.current) {
                    ...
                    noteTextareaRef.current.style.height =
                      Math.min(noteTextareaRef.current.scrollHeight, 200) + "px";
                  }
                });
              }}
