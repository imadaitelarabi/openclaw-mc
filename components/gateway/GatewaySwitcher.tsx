  ...
  return (
    <DropdownMenu.Root open={open} onOpenChange={setOpen}>
      <DropdownMenu.Trigger asChild>
        <button className={`flex items-center gap-1.5 px-2 py-1 rounded hover:bg-accent transition-colors outline-none ${
          status === 'connected' ? 'text-primary' : status === 'no-config' ? 'text-muted-foreground' : 'text-destructive'
        }`}>
          {status === 'connected' ? (
          ...
          <span className="font-medium">
            {status === 'connected' ? 'Connected' : status === 'no-config' ? 'No Gateway' : 'Disconnected'}
          </span>
          ...
          <div className="max-h-[300px] overflow-y-auto space-y-0.5">
            {gateways.map((gateway) => (
              <div key={gateway.id} className="flex items-center gap-1">
                ...
                >
                  <Globe className={`w-3.5 h-3.5 ${gateway.id === activeId ? 'text-primary' : 'text-muted-foreground'}`} />
                  <div className="flex flex-col">
                    <span className="font-bold">{gateway.name}</span>
                    <span className="text-[10px] opacity-50 truncate max-w-[140px]">{gateway.url}</span>
                  </div>
                  {gateway.id === activeId && <Check className="w-3.5 h-3.5 ml-auto text-primary" />}
                </DropdownMenu.Item>
