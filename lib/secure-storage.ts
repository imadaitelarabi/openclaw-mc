...
export class SecureStorage {
  ...
  static async setItem(extensionName: string, key: string, value: string): Promise<void> {
    ...
    try {
      ...
      localStorage.setItem(`${this.STORAGE_KEY_PREFIX}${extensionName}_${key}`, base64);
    } catch (error) {
  ...
  static async getItem(extensionName: string, key: string): Promise<string | null> {
    ...
    try {
      const stored = localStorage.getItem(`${this.STORAGE_KEY_PREFIX}${extensionName}_${key}`);
  ...
  static removeItem(extensionName: string, key: string): void {
    ...
    localStorage.removeItem(`${this.STORAGE_KEY_PREFIX}${extensionName}_${key}`);
  }
  ...
  static hasItem(extensionName: string, key: string): boolean {
    ...
    return localStorage.getItem(`${this.STORAGE_KEY_PREFIX}${extensionName}_${key}`) !== null;
  }
}
